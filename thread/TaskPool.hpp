...
class LocalTaskPool : public boost::noncopyable
{
  ...
  template<typename T>
    ...
    public:
      ...
      bool
      operator()(const T* const t, ThreadState state)
      {
        return StateLess(t->GetThreadState(), state);
      }
      bool
      operator()(ThreadState state, const T* const t)
      {
        return StateLess(t->GetThreadState(), state);
      }
    private:
      bool
      StateLess(ThreadState state1, ThreadState state2) const
      {
...
public:
  ...
  bool
  Init(size_t maxThreadNum = (size_t) boost::thread::hardware_concurrency());
  void
  Start();
  void
  Stop();
  inline void
  Run(CallableBase* task);
  inline bool
  Stopped() const
  {
    return m_isStopped;
  ...
  bool
  RemoveMostInsigfinicantTasks();
  void
  PushTask(CallableBase* task);
  CallableBase*
  PopTask();
...
bool
LocalTaskPool::Init(size_t maxThreadNum)
{
  bool ret = true;
  do
    {
  ...
  if (!m_isStopped)
    {
      ...
      std::cout << "Thread: " << m_threadMonitoring.GetThreadId()
          << " stopped\n";
...
inline void
LocalTaskPool::Run(CallableBase* task)
{
  if (!m_isStopped)
    {
      PushTask(task);
...
void
LocalTaskPool::PushTask(CallableBase* task)
{
  do
    {
      if (nullptr == task)
...
bool
LocalTaskPool::RemoveMostInsigfinicantTasks()
{
  TASK_LIST::iterator iter = std::find_if(m_taskQueue.begin(),
      m_taskQueue.end(), MostInsigfinicantTaskPred());
...
void
LocalTaskPool::CreateAndStartThread()
{
  // protect the whole transaction utilize lock_guard
  if (!m_isStopped)
    {
      WorkerThread* thr = new WorkerThread(THREAD_PRIORITY_ABOVE_NORMAL, /*auto-run=*/
      false);
